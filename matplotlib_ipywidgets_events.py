...
class App2(widgets.HBox):
    ...
    def handle_slider(self, change):
        ...
        self.ax.plot(np.random.random([a,b]), 'o', pickradius=50) # 5 points tolerance
    ...
    def init_plot(self):
        ...
        with self.output:
            self.fig, self.ax = plt.subplots()
        self.A, self.B = 100,2
        self.ax.plot(np.random.random([self.A, self.B]), 'o', pickradius=50) # 5 points tolerance set_pickradius
        self.fig.canvas.toolbar_position = 'left'
        self.ax.grid(True)
